...
$row = mysqli_fetch_assoc($res);
   ...
   if($oldname!=$name){
    ...
    if(!isset($error)){ 
          if($file!= "")
          {
            $stmt = mysqli_query($dbc,"SELECT image FROM  users WHERE account_id='$account_id'");
            $row = mysqli_fetch_array($stmt); 
            $deleteimage=$row['image'];
unlink($folder.$deleteimage);
move_uploaded_file($file, $folder . $new_image_name); 
mysqli_query($dbc,"UPDATE users SET image='$new_image_name' WHERE account_id='$account_id'");
          }
           $result = mysqli_query($dbc,"UPDATE users SET name='$name',password='$password', address= '$address' WHERE email = '$email' ");
           if($result)
           {
       header("location:my_account.php?profile_updated=1");
           }
           else 
           {
     ...
     <form method="post" enctype='multipart/form-data' action="">
          <div class="row">
           ...
           <div class="col-6"> 
            ...
            <div class="col"><p><a href="my_account.php"><span style="color:red;">Back</span> </a></p>
         </div>
          </div>
